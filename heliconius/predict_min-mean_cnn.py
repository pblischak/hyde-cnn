...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Options for predict_min-mean_cnn.py", add_help=True)
    ...
    data = np.load('heliconius_min-mean_data.npz')
    xtest,ytest = data['xtest'],data['ytest']
    del data

    model = load_model('heliconius_cnn_min-mean.mdl')
    pred = model.predict(xtest)
    pred_cat = [i.argmax() for i in pred]

    # Get prediction accuracy
    print(classification_report([np.argmax(ytest[i,:]) for i in range(ytest.shape[0])],
                      pred_cat, target_names=['no_hyb', 'admix', 'mig']))
    ...
    im, cbar = heatmap(np.array(cm_norm), ['no_hyb', 'admix', 'mig'],
                       ['no_hyb', 'admix', 'mig'], ax=ax,
                       cmap="Greys", vmin=0, vmax=1, cbarlabel="Accuracy")
    texts = annotate_heatmap(im, valfmt="{x:.3f}")
    fig.tight_layout()
    plt.savefig("heliconius_cnn_min-mean_confusion-matrix.svg")
    plt.savefig("heliconius_cnn_min-mean_confusion-matrix.png")
    plt.show()
